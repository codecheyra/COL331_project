     ...
     for(int j=0; j<NPDENTRIES; j++){
      if (pde[j] & PTE_P){
        ...
        for(int m=0; m<NPTENTRIES; m++){
          if (PTE_ADDR(ans[m])==PTE_ADDR(*pte)){
            if (p->rss>0){p->rss-=PGSIZE; break;}
            // release(&ptable.lock);
            // return;
          }
        }
...
struct proc* victim_proc(){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
        ...
        if(p->rss > max_rss || (p->rss>=max_rss && p->pid < victim_proc->pid)){
            victim_proc = p;
            max_rss = p->rss;
            // cprintf("victim pid in proc.c %d\n",p->pid);
        }
    }
    release(&ptable.lock);
    return victim_proc;
...
//     uint max_rss = -1;
//     struct proc *victim_proc = ptable.proc;
//     uint maxi_id=victim_proc->pid;
//     acquire(&ptable.lock);
//     for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
//         if(p->state == UNUSED){continue;}
//         if(p->rss > max_rss || (p->rss>max_rss && p->pid < maxi_id)){
//             // victim_proc = p;
//             max_rss = p->rss;
//             maxi_id=p->pid;
//             // cprintf("victim pid in proc.c %d\n",p->pid);
...
//     }
//     release(&ptable.lock);
//     struct proc* ans=&ptable.proc[maxi_id];
//     return ans;
// }
