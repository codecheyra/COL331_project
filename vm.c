...
void decre_rev_map(uint pa, pte_t* pte){
    ...
    if (*pte & PTE_S){
      ...
      int count=0;
      for(int i=0; i<NPROC; i++){
        if (sblocks[blockid].fill[i]==0 && sblocks[blockid].pte_entry[i]==pte){
          sblocks[blockid].fill[i]=1;
          count+=1;
        }
        else if (sblocks[blockid].fill[i]==1){
          count+=1;
        }
      }
      if (count>=NPROC){
    ...
    if (rev_map[pa/PGSIZE].pg_ref_count<0){
      panic("error in decrement rev map\n");
    }
    release(&(rev_map[pa/PGSIZE].lock));
    decre_rss(pte);
    return;
...
void swap_page(){
  ...
  acquire(&(rev_map[pa/PGSIZE].lock));
  for(int i=0; i<NPROC; i++){
    if (rev_map[pa/PGSIZE].fill[i]!=-1 && PTE_ADDR(*rev_map[pa/PGSIZE].pid_arr[i])==pa ){
      pte_t* che=rev_map[pa/PGSIZE].pid_arr[i];
      for(int j=0; j<NPROC; j++){
        if (sblocks[blockid].fill[j]){
          sblocks[blockid].fill[j]=0;
          sblocks[blockid].page_perm[j]=PTE_FLAGS(*che);
          *che=blockid<<12|PTE_S;
          sblocks[blockid].pte_entry[j]=che;
          break;
        }
      }
    }
  ...
  //                 *che=blockid<<12|PTE_S;
  //                 // cprintf("swapped friend page pte %d\n",*che);
  ...
  for(int i=0; i<NPDENTRIES; i++){
    if (pde[i] & PTE_P){
      ...
      for(int j=0; j<NPTENTRIES; j++){
        if (ans[j] & PTE_S){
          uint blockid=(ans[j])>>12;
          int count=0;
          for(int i=0; i<NPROC; i++){
            if (sblocks[blockid].fill[i]==0 && sblocks[blockid].pte_entry[i]==&ans[j]){
              sblocks[blockid].fill[i]=1;
              count+=1;
            }
            else if (sblocks[blockid].fill[i]==1){
...
void handle_pgfault(){
  ...
  // if (V2P(va)>KERNBASE){
  //   cprintf("kernbase %d %d\n",*pte, V2P(va));
  //   // panic("error\n");
  // }

  // cprintf("in pagefault %d %d %d %d %d\n",p->pid , (*pte & PTE_S), (*pte & PTE_W), (*pte), PTE_ADDR(*pte));
  if (*pte & PTE_S){
    uint blockid= (*pte) >> 12;
    // cprintf("swapblock %d %d\n",sblocks[blockid].is_free,blockid);
    char* mem=kalloc();
    // cprintf("swapblock %d\n",sblocks[blockid].is_free);
    mem=read_swap_page(mem,2+(8*blockid));
    // cprintf("swapblock %d\n",sblocks[blockid].is_free);
    uint pa=PTE_ADDR(V2P(mem));
    for(int i=0; i<NPROC; i++){
      if (sblocks[blockid].fill[i]==0){
        // increment_rss(sblocks[blockid].arr[i]); // use pte to increase rss;
        incre_rss(sblocks[blockid].pte_entry[i]);
    ...
    if (*pte & PTE_W){
      ...
      if (refcount>1){
        ...
        *pte |=PTE_W;
        // cprintf("new: %d and old %d %d %d %d\n",refcount, *pte, *pte & PTE_A, *pte & PTE_P);
      }
      lcr3(V2P(p->pgdir));
  }
}
